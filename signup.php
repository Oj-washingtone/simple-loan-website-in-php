...
require_once('controller/config.php');
 	...
 	if($_POST['submit']){
 		...
 		if(empty($firstName) || empty($surname) || empty($phone) || empty($id_number) || empty($password) || empty($confirmPassword)){
 			...
 			if($phoneLength != 10){
 				$error[] = "your phone number must be 10 digits";
 				print $error[0];
 			}
 			if($password != $confirmPassword){
 				$error[] = "the passwords you entered are not matchin";
 				print $error[0];
 			}else{
	 			$password_len = strlen($password);
	 			if($password_len < 8){
	 				$error[] =  "password must me more than 8 characters";
	 				print $error[0];
	 			}
	 		}
	 		...
	 		if($error == 0){
	 			...
	 			if($rows == 1){
	 				$error[] = "User with the same ID number already exists in our database";
	 				print $error[0];
	 			}else{
