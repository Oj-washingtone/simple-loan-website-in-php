...
function userLoan($userid){
  ...
  $result = $connection->query($loanQuery);
    ...
    //check number of riws returned
    $row = $result->num_rows;
    if($row == 0){
    $loan = false;
    }else{
    //  check the repayments table to see if the loan has been payed
    ...
    $repaymentQuery = "SELECT loanId FROM loanRepayments WHERE loanId = '".$loanid."' ORDER BY date DESC LIMIT 1;";
